# Page snapshot

```yaml
- banner:
  - heading "Scrypture" [level=1]
  - paragraph: Grimorium Vivendi
  - text: Achievement Test Use Level 1
  - button "📊"
  - button "🏆 1/15"
- main:
  - text: 🏡
  - heading "Dam & Sanctuary" [level=2]
  - button "▼"
  - button "Start Here"
  - text: Tasks Completed 1 / 5
  - heading "Core Attributes" [level=3]
  - text: 💪 Body 0 🧠 Mind 0 ✨ Soul 0 Level 1 80 / 100
  - textbox "Intention"
  - heading "Active Tasks" [level=2]
  - textbox "Search tasks..."
  - combobox:
    - option "All Categories" [selected]
    - option "💪 Body"
  - combobox:
    - option "⚡ Priority" [selected]
    - option "📅 Date"
    - option "⭐ XP"
  - button "Sort descending": ↑
  - button "Collapse Uncategorized category":
    - text: 📝
    - heading "Uncategorized" [level=3]
    - text: 1 task ▲
  - heading "Progress Task 1" [level=3]
  - text: "Medium Priority just now XP: +10"
  - checkbox
  - button "Edit task": 🖍
  - button "Collapse body category":
    - text: 💪
    - heading "Body" [level=3]
    - text: 3 tasks ▲
  - heading "Progress Task 2" [level=3]
  - text: "Medium Priority just now XP: +10"
  - checkbox
  - button "Edit task": 🖍
  - heading "Progress Task 3" [level=3]
  - text: "Medium Priority just now XP: +10"
  - checkbox
  - button "Edit task": 🖍
  - heading "Progress Task 4" [level=3]
  - text: "Medium Priority just now XP: +10"
  - checkbox
  - button "Edit task": 🖍
  - heading "Completed Tasks (1)" [level=2]
  - text: ▲
  - heading "Progress Task 5" [level=3]
  - text: "Medium Priority just now XP: +10"
  - checkbox [checked]
  - button "Edit task": 🖍
  - heading "Habits" [level=2]
  - button "Data Manager ▲"
  - heading "🔐 Secret Menu" [level=3]
  - button "🔒 LOCKED"
- button "×"
- text: 🌱 Achievement Unlocked!
- heading "First Steps" [level=3]
- paragraph: Bóbr nods approvingly. Your dam-building journey has begun!
- text: common +50 XP 0
```