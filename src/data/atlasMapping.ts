/**
 * Atlas Mapping for Framesandbuttonsatlas.png
 * Comprehensive data ID set for all UI assets in the sprite sheet
 */
...
/**
 * Complete Atlas Mapping for Framesandbuttonsatlas.png
 * Based on the sprite sheet organization:
 * - Rows 1-2: Small buttons (16x16) - Default and Active states
 * - Rows 3-4: Wide buttons (16x32) - Default and Active states  
 * - Rows 5-6: Large buttons (64x32) - Default and Active states
 */
...
export const getSpritesByCategory = (category: 'button' | 'frame'): AtlasSprite[] => {
...
export const getButtonSprites = (size?: 'small' | 'wide', theme?: string, state?: string): AtlasSprite[] => {
  return ATLAS_MAPPING.sprites.filter(sprite => {
    if (sprite.category !== 'button') return false;
    if (size && sprite.subcategory !== size) return false;
